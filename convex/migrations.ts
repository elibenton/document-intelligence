...
/**
 * Strip the dead render-lifecycle fields so the schema can stop declaring
 * them. The server geometry pipeline was deleted 2026-08-18 (geometry comes
 * from OCR precontext, pixels from client pdf.js); Convex rejects rows
 * carrying fields the schema no longer declares, so the data goes first —
 * widen → migrate → narrow. Run once per deployment:
 *   npx convex run migrations:stripRenderFields
 * then delete the documents.render* block and pages.geometryVersion from
 * schema.ts. Idempotent; paginated and self-rescheduling like its neighbours.
 */
export const stripRenderFields = internalMutation({
  args: { cursor: v.optional(v.union(v.string(), v.null())) },
  returns: v.null(),
  handler: async (ctx, args) => {
    const page = await ctx.db
    ...
    for (const doc of page.page) {
      if (
        doc.renderStatus === undefined &&
        doc.renderScheduledAt === undefined &&
        doc.renderExpectedPages === undefined &&
        doc.renderedPageCount === undefined &&
        doc.rendererVersion === undefined &&
        doc.renderLastError === undefined &&
        doc.renderAttempts === undefined &&
        doc.renderStartedAt === undefined &&
        doc.renderCompletedAt === undefined
      )
        continue;
      await ctx.db.patch(doc._id, {
        renderStatus: undefined,
        renderScheduledAt: undefined,
        renderExpectedPages: undefined,
    ...
    if (!page.isDone) {
      await ctx.scheduler.runAfter(0, internal.migrations.stripRenderFields, {
        cursor: page.continueCursor,
      });
    } else {
      await ctx.scheduler.runAfter(0, internal.migrations.stripPageGeometryVersion, {});
    }
    return null;
  },
});

/** Second half of stripRenderFields: pages.geometryVersion. */
export const stripPageGeometryVersion = internalMutation({
  args: { cursor: v.optional(v.union(v.string(), v.null())) },
  returns: v.null(),
  handler: async (ctx, args) => {
    const page = await ctx.db
    ...
    for (const row of page.page) {
      if (row.geometryVersion === undefined) continue;
      await ctx.db.patch(row._id, { geometryVersion: undefined });
    }
    if (!page.isDone) {
      await ctx.scheduler.runAfter(
        0,
        internal.migrations.stripPageGeometryVersion,
        { cursor: page.continueCursor }
      );
    }
    return null;
